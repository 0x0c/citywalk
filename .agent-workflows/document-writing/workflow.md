...
citywalk's writing is argued prose: roadmap items and the documents under `docs/` make a case a reader
...
- **An umbrella above two language layers.** The *language-agnostic principles* below hold for both
  ...
  for its language.
- **Enforcement stays a review-time norm, not a continuous-integration gate.** Judging clarity,
  emphasis, and argument order needs semantic judgment, and a large language model on the merge gate
  would make the build verdict non-reproducible. The norm holds the way the bilingual-documentation
  norm holds: a contributor and reviewer expectation, checked by people.
...
- **State the contribution up front.** A roadmap item's Introduction and Motivation state the problem,
...
repository — no other roadmap item, no other page under `docs/`, no earlier commit — must still be able
