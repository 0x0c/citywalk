...
[**ロードマップサイト**](https://0x0c.github.io/citywalk/)は、全項目を1つのページに表示します。
各項目の状態、トピックごとの実装済みの割合、項目内のチェックリストの進み具合、項目どうしの関連図を
見渡せます。ページはビルド時にこのディレクトリを読み取り、プッシュのたびに公開し直すため、ここに
あるファイルから遅れることはありません。手元で作るときは `make site` を、その場で表示するときは
`make serve` を実行します。
...
- [`scripts/build_roadmap_site.py`](../scripts/build_roadmap_site.py)：ロードマップサイトを生成する
  スクリプトです。メタデータの書式を変えるときは、この読み取り側もあわせて直します。
